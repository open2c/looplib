    ...
    cdef np.int64_t check_system(System self):
        ...
        for i in range(self.N):
            ...
            if not (
                (
                    (self.locs[i] < 0) 
                    and (self.locs[i+self.N] < 0 )
                    and (self.locs[i+2*self.N] < 0 )
                    and (self.locs[i+3*self.N] < 0 )
                ) or(
                    (self.locs[i] >= 0) 
                    and (self.locs[i+self.N] >= 0 )
                    and (self.locs[i+2*self.N] >= 0 )
...
cdef regenerate_event(System system, Event_heap evheap, np.int64_t event_idx):
    ...
    # Passive unbinding.
        ...
        if (system.locs[loop_idx] >= 0) and (system.locs[loop_idx+system.N] >= 0):
            t_step = rand_exp(system.lifespans[loop_idx])
...
cpdef simulate(p, verbose=True):
    ...
    cdef np.int64_t [:,:] l_sites_traj = -1 * np.ones((N_SNAPSHOTS, N), dtype=np.int64) 
    cdef np.int64_t [:,:] r_sites_traj = -1 * np.ones((N_SNAPSHOTS, N), dtype=np.int64) 
    cdef np.int64_t [:,:] centers_traj = -1 * np.ones((N_SNAPSHOTS, 2*N), dtype=np.int64) 
    ...
    for i in range(system.N):
        ...
        # regenerate all of its events and the motion of its neighbours
            ...
            regenerate_neighbours(system, evheap, INIT_LOCS[i+2*system.N])
            regenerate_neighbours(system, evheap, INIT_LOCS[i+3*system.N])
    ...
    while snapshot_idx < N_SNAPSHOTS:
        ...
        if system.time > prev_snapshot_t + T_MAX / N_SNAPSHOTS:
            ...
            r_sites_traj[snapshot_idx] = system.locs[N:2*N]
            centers_traj[snapshot_idx] = system.locs[2*N:4*N]
    ...
    return np.array(l_sites_traj), np.array(r_sites_traj), np.array(centers_traj), np.array(ts_traj)
